...
class PixabayMusicDownloader:
    ...
    def parse_multiple_pages(self, base_url: str, start_page: int = 1, end_page: int = 3) -> List[Dict]:
        """
        Parse nhiều trang Pixabay với pagination từ start_page đến end_page
        """
        all_music_items = []
        successful_pages = 0
        ...
        for page_num in range(start_page, end_page + 1):
            try:
                ...
                if page_items:
                    ...
                    start_index = len(all_music_items) + 1
                    for i, item in enumerate(page_items):
                        item['index'] = start_index + i
                        item['page'] = page_num
                    
                    all_music_items.extend(page_items)
                    successful_pages += 1
                    print(f"✅ Trang {page_num}: Thêm {len(page_items)} tracks")
                else:
                    print(f"❌ Trang {page_num}: Không tìm thấy tracks")
                    # Nếu trang trống, có thể đã hết
                    if page_num > start_page:
                        print("💡 Có thể đã đến trang cuối, dừng crawling")
                        break
                
                # Delay giữa các trang
                if page_num < end_page:
                    print(f"⏳ Chờ 3s trước khi crawl trang tiếp...")
                    time.sleep(3)
...
Hoặc nhập URL khác để thử:""".format(url=url))
    ...
    def download_music_range(self, start_idx: int, end_idx: int, download_folder: str = "downloads"):
        """
        Download nhạc theo range từ start_idx đến end_idx
        """
        if not self.music_list:
        ...
        print(f"\n🚀 Bắt đầu download từ {start_idx} đến {end_idx}")
        ...
        for i in range(start_idx - 1, end_idx):
            ...
            print(f"\n⬇️  Đang download {item['index']}: {item['title']}")
            
            try:
                # Làm sạch tên file
                safe_title = re.sub(r'[<>:"/\\|?*]', '_', item['title'])
                # Sử dụng số thứ tự mới để tránh ghi đè
                new_file_number = next_file_index + (i - (start_idx - 1))
                filename = f"{new_file_number:03d}_{safe_title}.mp3"
                ...
                # Download file
                print(f"   🌐 Downloading từ: {real_url}")
                response = self.session.get(real_url, stream=True, timeout=30)
                response.raise_for_status()
                
                # Kiểm tra content type
                content_type = response.headers.get('content-type', '')
                if 'audio' not in content_type.lower() and 'mpeg' not in content_type.lower():
                    print(f"⚠️  Cảnh báo: File có thể không phải MP3 (Content-Type: {content_type})")
                ...
                print(f"✅ Hoàn thành: {filename} ({size_str})")
                success_count += 1
                
                # Tạm dừng để tránh bị block
                time.sleep(1)
                
            except Exception as e:
                print(f"❌ Lỗi download {item['title']}: {e}")
                failed_count += 1
                continue
        
        print(f"\n" + "="*50)
        print(f"📊 KẾT QUẢ DOWNLOAD")
...
def main():
    ...
    if crawl_choice == '2':
        try:
            ...
            music_list = downloader.parse_multiple_pages(url, start_page, end_page)
        except ValueError:
            print("❌ Số không hợp lệ, dùng mặc định trang 1-3")
            music_list = downloader.parse_multiple_pages(url, 1, 3)
    else:
...
Chọn (1/2/3): """).strip()
    ...
    try:
        ...
        if confirm in ['y', 'yes']:
            downloader.download_music_range(start, end, folder)
        else:
            print("❌ Đã hủy download.")
